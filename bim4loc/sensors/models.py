...
@njit(cache = True)
def inverse_lidar_model_new(wz_i, sz_i, szid_i, beliefs, 
    ...
    for j in prange(valid_hits):
        ...
        if szid_i[j] == len(beliefs):
            inv_eta +=  Pjplus * forward_lidar_model(wz_i, sz_ij,sensor_std, sensor_max_range)
            # inv_eta +=  Pjplus * delta(wz_i, sensor_max_range)
            Pjplus = Pjbar * 1.0
