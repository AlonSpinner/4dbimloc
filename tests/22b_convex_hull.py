...
drone = Drone(pose = np.array([2.0, 2.5, 2.7/2, 0 * np.pi/6]))
...
for i, ray_uv in enumerate(hit_ray_uv):
    weight_dist_2_boundry[i], _ = minimal_distance_from_projected_boundry(ray_uv, element_uv)
...
ax.plot(element_uv_hull_plus[:,0],element_uv_hull_plus[:,1])
